...
int MLModel::init()
{
    ...
    static tflite::MicroMutableOpResolver<5> _opsResolver;  // NOLINT
    _opsResolver.AddMaxPool2D();
    _opsResolver.AddConv2D();
    _opsResolver.AddFullyConnected();
    _opsResolver.AddSoftmax();
    _opsResolver.AddReshape();

    _interpreter = new tflite::MicroInterpreter(_model, 
                                                _opsResolver,
                                                _tensor_arena, 
                                                _tensor_arena_size,
                                                &_error_reporter);
    ...
    // Define the shape of input and output to fit the model
    _input_tensor = _interpreter->input(0);

    TfLiteIntArray* input_dims = _input_tensor->dims;
...
float MLModel::predict()
{
    ...
    if (invoke_status != kTfLiteOk) {
        printf("Failed to invoke the mode!\n");
        return NAN;
    }
    ...
    float y_estimate = _output_tensor->data.f[1];
    // printf("Prediction 0 - %f, 1 - %f\n", _output_tensor->data.f[0], _output_tensor->data.f[1]);
